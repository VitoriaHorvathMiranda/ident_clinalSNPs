...
#11994070 - 2L
pop_test <- fread("/dados/time_clines/output/SNPs/joined97_mincount5_minfreq0.005_cov10.tsv")

pop_test_lite <- pop_test[CHROM == "2L",]
rm(pop_test)
pop_test_lite[, position2 := paste(CHROM, POS, sep = ":")]

pop_test_lite <- pop_test_lite[, !c("CHROM", "POS")] 

pop_test_lite %>%
  group_by(position2, population) %>%
  summarise(mean_freq = mean(freq)) %>%
  filter(mean == 0)

pop_test_lite[,mean := mean(freq), by = c("position2", "population")]
pop_test_lite[mean != 0,]

no_freq_lite <- pop_test_lite[mean == 0,][, .(unique(position2))]
